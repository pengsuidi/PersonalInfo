...
public class order_shop_Adapter extends RecyclerView.Adapter<order_shop_Adapter.orderviewHolder> {
    ...
    private List<payment> datas = new ArrayList<>();

    private RecyclerView.Adapter adapter;
    private List<payment> Alldatas = new ArrayList<>();
    private List<payment> orderShopList = new ArrayList<>();
    ;

    public order_shop_Adapter(List<payment> datas, SharedPreferences sharedPreferences, Context context, List<payment> Alldatas) {
    ...
    public void onBindViewHolder(@NonNull final orderviewHolder holder, int position) {
        final payment order_shop = datas.get(position);


        Bitmap tmp = getBitmapFromByte(order_shop.getShop_img());
