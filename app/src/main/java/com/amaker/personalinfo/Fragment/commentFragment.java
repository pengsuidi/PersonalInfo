...
//                        System.out.println("result:" + result);
                        ...
                        comments = jsonArray.toJavaList(UserComment.class);
                        System.out.println("评论个数:" + comments.size());
                        System.out.println("分数:" + comments.get(0).getComment_grade());
                        System.out.println("评论:" + comments.get(0).getComment());

                        //更新商家得分
                        Float sum = 0.0f;
                        for (int i = 0; i < comments.size(); i++) {
                            if (comments.get(i).getComment_grade() == null) {
                                break;
                            }
                            System.out.println("分数:" +comments.get(i).getComment_grade());
                            sum += comments.get(i).getComment_grade();
                        }
                        if (comments.size() == 0) {
        ...
        switch (view.getId()) {
            ...
            case R.id.good:
                ...
                for (int i = 0; i < comments.size(); i++) {
                    ...
                    if (comments.get(i).getComment_grade() >= good_comment) {
                        datas.add(comments.get(i));
                    }
                }
                System.out.println("点击后大小:"+datas.size());
            ...
            case R.id.bad:
                ...
                for (int i = 0; i < comments.size(); i++) {
                    //datas中只放构造Recyclerview需要的数据,即分别为"all","good","bad"时的数据
                    if (comments.get(i).getComment_grade() <= bad_comment) {
                        datas.add(comments.get(i));
                    }
                }
                System.out.println("点击后大小:"+datas.size());
