                ...
                case Config.ADD_FOOD://网络请求成功，但响应需要根据实际来操作
                    ...
                    for (int i = 0; i < FoodDatas.size(); i++) {
                        if (FoodDatas.get(i).getFood_name() == msg.obj.toString()) {
                            ...
                            sum += FoodDatas.get(i).getFood_price();
                            tmp_sum.setText("¥" + sum);
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            ...
            case R.id.to_shoppingcar:
                ...
                for (int i = 0; i < OrderedFoodList.size(); i++) {
                    food_id_list.add(String.valueOf(OrderedFoodList.get(i).getFood_id()));
                }
                intent1.putStringArrayListExtra(Config.FOOD_ID_LIST, food_id_list);
